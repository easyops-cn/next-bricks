...
import { previewStart } from "./preview-start.js";

jest.mock("@next-core/runtime");
jest.mock("./preview/inspector.js");
jest.mock("./preview/capture.js");
...
jest
  ...
  .mockImplementation();
jest.spyOn(runtime.__secret_internals, "updateStoryboardByRoute").mockImplementation();
jest
  .spyOn(runtime.__secret_internals, "updateStoryboardByTemplate")
  .mockImplementation();
jest.spyOn(runtime.__secret_internals, "updateStoryboardBySnippet").mockImplementation();
// jest.spyOn(runtime.__secret_internals, "updateFormPreviewSettings").mockImplementation();
jest.spyOn(runtime, "matchPath").mockImplementation((pathname, options) => pathname === options.path ? {} as any : null);
...
describe("previewStart", () => {
  it("should work", async () => {
    ...
    // Ignore re-start.
    previewStart("http://localhost:8081", { appId: "test" });
    expect(parentPostMessage).toBeCalledTimes(5);
    ...
    expect(runtime.__secret_internals.updateStoryboard).toBeCalledWith("my-app", {
      routes: [],
    });
    ...
    expect(runtime.__secret_internals.updateTemplatePreviewSettings).toBeCalledWith(
      "my-app",
      "my-tpl",
      {
        properties: { dataTest: "good" },
    ...
    expect(runtime.__secret_internals.updateStoryboardByRoute).toBeCalledWith("my-app", {
      routes: [],
    });
    ...
    expect(runtime.__secret_internals.updateStoryboardByTemplate).toBeCalledWith(
      "my-app",
      {
        routes: [],
      },
