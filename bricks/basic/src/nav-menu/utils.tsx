...
export function isSimple(item: MenuItem): item is MenuSimpleItem {
...
export const renderSpanCom = (item: MenuGroup): React.ReactElement => {
  ...
  if (!match && Array.isArray(item.activeIncludes)) {
    for (const include of item.activeIncludes) {
      ...
      match = !!matchPath(pathname, {
        path: hasSearch
          ? (parseInclude as Location<unknown>).pathname
          : include,
        exact: true,
      });
