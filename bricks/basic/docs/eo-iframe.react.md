...
```tsx
import { WrappedEoIframe } from "@easyops/wrapped-components";
```
...
```tsx
<WrappedEoIframe src="https://example.com" iframeStyle={{ height: "400px" }} />
```

### 监听加载完成事件

页面加载完成后执行操作，通过监听 `onLoad` 事件获知 iframe 完成加载。

```tsx
<WrappedEoIframe
  src="https://example.com"
  iframeStyle={{ height: "400px" }}
...
<>
  <WrappedEoIframe
    ...
    iframeStyle={{ height: "400px" }}
  />
  <button onClick={() => iframeRef.current?.postMessage("hello iframe", "*")}>
    发送消息
  </button>
</>;
```
