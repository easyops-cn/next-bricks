...
export function ComplexSearchComponent(props: ComplexSearchProps): React.ReactElement {
    const {
        value,
        onInputChange,
        onSearch,
    ...
    const handleClick = (event: MouseEvent) => {
        const targetElement = event.composedPath().some(ele => ele === contentRef.current);
        if (!targetElement) {
            setVisible(false);
        }
    }
    const handleMouseEnter = (e:React.MouseEvent<HTMLDivElement>, data:OptionItem) => {
        setCurrentData(data);
        setTooltipVisible(true);
        const top = (e.target as HTMLDivElement).getBoundingClientRect().top - contentRef.current.getBoundingClientRect().top;
        setPositionTop(top);
    ...
    const TooltipBrick = useMemo(() => {
        if (!tooltipUseBrick?.useBrick) {
            ...
            return (
                <div className="dropdownMenuItemTooltip"
                     style={{
                         top: positionTop,
                    ...
                    <div className="useBrickWrapper">
                        <ReactUseMultipleBricks useBrick={tooltipUseBrick.useBrick} data={currentData||{}} />
                    </div>
                </div>)
        }
    ...
    return <div className="wrapper" ref={contentRef}>
        <div className="inputWrapper">
            ...
            />
            <WrappedIcon className="searchIcon icon" lib="antd" icon="search" theme="outlined"/>
            {allowClear &&
                <WrappedIcon className="clearIcon icon" onClick={onClearValue} lib="antd" icon="close-circle"
                             theme="filled"/>}
        </div>
                        ...
                        <div className="dropdownMenu">
                            {
                                options.map((option, index) => (
                                    <div className="dropdownMenuItem" key={index}
