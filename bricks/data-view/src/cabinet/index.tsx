...
class CabinetGraph extends ReactNextElement implements CabinetGraphProps {
  ...
  */
  ...
  #handleActiveKeyChange = (newKey: string) => {
    ...
    <div className="wrapper" onClick={(e) => {
      ...
      <div className="container">
        <WrappedCabinetAppLayer className="app-layer" appTitle={dataSource.appName}
          onClick={(e) => {
            e.stopPropagation();
            onActiveKeyChange(dataSource.key);
          }}
          status={activeData.type === "layer" && activeData.layer == dataSource.key ? "active" : undefined}
        />
          ...
          {dataSource.clusters.map(clu => {
            return <WrappedCabinetContainer key={clu.key} type={clu.type} customTitle={clu.clusterName}
              data={clu.nodes.map(node => ({
                ...
                status: (() => {
                  switch (activeData.type) {
                    case "cluster": {
                      return activeData.cluster === clu.key ? undefined : "faded";
                    }
                    case "node": {
                      return activeData.node === node.key ? "active" : "faded";
                    }
                    default:
                      return undefined;
                  }
              ...
              handleClick={({ type, data }) => {
                switch (type) {
                  case "container": {
                    onActiveKeyChange(clu.key);
                    break;
                  }
                  case "node": {
                    onActiveKeyChange((data as any).key);
                    break;
                  }
                }
              }}
              status={(() => {
                switch (activeData.type) {
                  case "cluster": {
                    return activeData.cluster === clu.key ? "active" : "faded";
                  }
                  case "node": {
                    return activeData.cluster === clu.key ? undefined : "faded";
                  }
