...
const OTHER_BUBBLE_MAX_RADIUS = 162;
...
export function BubblesIndicatorComponent({
  ...
  const labels = useMemo(() => {
    const nodes = [centerDataSource, ...dataSource].map<ForceNode>(
      (item, index) =>
        index === 0
          ? {
          ...
          : {
              r: Math.min(
                ...
                Math.sqrt(
                  (item.value as number) / (centerDataSource.value as number)
                ) * CENTER_BUBBLE_RADIUS
              ),
              // r: item.value as number / (centerDataSource.value as number) * CENTER_BUBBLE_RADIUS,
            }
    );
    ...
    const simulation = forceSimulation(nodes)
      ...
      .force(
        "charge",
        forceManyBody().strength((_d, i) => (i ? 0 : (-BASE_WIDTH * 1) / 2))
      );
