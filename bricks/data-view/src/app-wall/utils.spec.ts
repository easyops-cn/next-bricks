...
describe("utils", () => {
  test.each([
    ...
    expect(
      getCenterPointOrSubPoint(
        data.start as Vector3Tuple,
        data.end as Vector3Tuple
      )
  ...
  test("createTrapezoidalObject & createTrapezoidalRightOrLeftElement", () => {
    expect(
      createTrapezoidalTopOrBottomElement({ ...trapezoidalParams, isTop: true })
    ).toHaveProperty("position", new Vector3(0, -60, 115));
    expect(
      createTrapezoidalTopOrBottomElement({
        ...trapezoidalParams,
        isTop: false,
    ...
    ).toHaveProperty("position", new Vector3(-112.5, 0, 115));
    expect(
      createTrapezoidalRightOrLeftElement({
        ...trapezoidalParams,
        isLeft: false,
  ...
  test("createRelationLine", () => {
    const result = createRelationLine(
      new Vector3(0, 0, 1),
      new Vector3(1, 0, 2),
      "blue"
    );
...
describe("findElementByEvent", () => {
  ...
  test("should return null when no matching element is found", () => {
    ...
    const result = findElementByEvent(event, "data-view.app-wall-card-item");
